...
class TheWrap(AbstractSource):
    def __init__(self) -> None:
        self.domain = "https://www.thewrap.com"
    ...
    def get_stories(self, soup: BeautifulSoup) -> List[Dict[str, Optional[str]]]:
        ...
        stories = []
        story_elements = soup.find_all('li', class_='wp-block-post')
        for story in story_elements:
            story_data = {}
            # Extract title and URL
            title_element = story.find('h2', class_='wp-block-post-title')
            if title_element and title_element.find('a'):
                story_data['title'] = title_element.find('a').get_text(strip=True)
                story_data['url'] = title_element.find('a').get('href')

            # Extract image URL
            image_element = story.find('div', class_='wp-block-post-featured-image').find("img")
            if image_element:
    ...
    def get_headline(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        title_element = soup.find('h1', class_='wp-block-post-title')
        if title_element:
    ...
    def get_banner_image(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        heading_image = soup.find('img', class_="wp-post-image")
        if heading_image:
    ...
    def get_author(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        author_element = soup.find('div', class_="wp-block-post-author")
        if author_element:
    ...
    def get_date(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        date_element = soup.find('div', class_="wp-block-post-date")
        if date_element:
